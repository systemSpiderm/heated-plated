...
# define N 500
  ...
  printf ( "  Number of processors available = 8\n" );
...
#pragma omp parallel num_threads(8) shared ( w ) private ( i, j ) 
  {
#pragma omp for
    for ( i = 1; i < M - 1; i++ )
    {
...
*/
#pragma omp parallel num_threads(8) shared ( mean, w ) private ( i, j ) 
  {
#pragma omp for
    for ( i = 1; i < M - 1; i++ )
    {
...
*/
  ...
  while ( epsilon <= diff )
  {
# pragma omp parallel num_threads(8) shared ( u, w ) private ( i, j ) 
    {
/*
...
*/
    diff = 0.0;
# pragma omp parallel num_threads(8) shared ( diff, u, w ) private ( i, j, my_diff )
    {
      my_diff = 0.0;
# pragma omp for
      for ( i = 1; i < M - 1; i++ )
